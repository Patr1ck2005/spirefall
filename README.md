...
- **Hostile mob waves (M31), opt-in.** The lobby's `Hostile mobs` switch (host-only) turns each sector's mob spawn anchors live: roughly every 11 seconds a ground grate telegraphs with a light pillar for one second — deliberately far from the nearest pilot — and an industrial pest climbs out. Three species share one family language (bodies drawn from the host map's panel shades, species-constant signal colors): the **Skitter Saw** is a ground crawler that stalks your shelf, heats its blade rim as a visible wind-up, then dashes; the **Ion Gnat Swarm** is a gravity-free drone cloud that drifts over cover, orbits at hover range and nibbles on contact; the **Ram Hauler** is a wheeled charger that rears up behind a red warning cone before a 400px/s rush that ends hard against any wall. Mobs hurt everyone (bots included), never count toward win conditions, soak damage from every weapon, blasts, barrels and lethal machinery, and drop an 8-second repair pack on death that restores every limb. Their eyes and blade heat glow — and the wind-up cone even casts its own shadow.
- **Pocket items (M32) — one G-slot, five tools.** Item crates (roughly one in five supply spawns, marked with their own glyph and signal color) grant a pocket item: the **grenade** arcs on a real throw and detonates under rocket power with the biggest knockback in the game; the **flashbang** pops a blinding burst — line of sight, distance, and facing all matter (staring at it zeroes you, looking away stings, cover blocks it, and the thrower is immune); the **medkit** restores every limb instantly; the **diffraction shield** is a held device that eats laser-family shots (beams and rails) from the front and splits each blocked shot into a 7-beam spectral fan — purple/green/red diffraction orders around a mirror beam that keeps the attacker's color — dealing the incoming damage ÷ 7 per beam to whatever it hits, attacker included; and the **jetpack** burns 2.2 seconds of server-authoritative fuel while `Shift` is held, then vanishes. Blindness is a private whiteout (each pilot's own overlay), and the **Cutter Blade's dash-slash flashes lightning-bright on connect** — everyone but the wielder eats the blind. Bots use their items too: medkits when ground down, shields under fire, grenades at visible mid-range targets.
...
- `G`: use the pocket item (grenade, flashbang, medkit, shield, jetpack fuel tank)
- `Shift` (hold): jetpack thrust while fuel remains
