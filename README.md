...
- **Industrial-poster art with a real lighting engine (M26).** The style is flat, confident color fields — every scene plate, platform and portrait is drawn procedurally in code (zero image assets). Depth comes from actual light: the plates carry Sobel-derived normal maps and render through Phaser's Light2D pipeline with a poster-graded shader, so every muzzle flash, explosion, barrel fire, searchlight and lightning strike pours per-pixel light onto the walls around it. Pilots are lit by the room, not by a flashlight: the strongest nearby light warms their armor and paints a rim stroke on the lit side of the helmet. Fortress sweeps two searchlight cones that cast true occlusion shadows; the foundry furnace breathes from below; the canopy storms with strobe lightning and rolling thunder. An adaptive governor sheds shadows, then engine lights, then the whole pass when frames run long, and the system is toggleable under `FX` ("Dynamic lighting / 动态光影").
