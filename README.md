...
- **Explosive barrels (M25).** Four destructible drums per sector take damage from every attack — bullets, beams, blades, and rocket splash (which chains between them). A barrel cooks off for 32 damage in a 70px blast with rocket-style falloff, glows through leaking cracks as it takes damage, and respawns after 6-10s. Barrels never block movement or shots; kills through barrels attribute through the normal kill feed.
...
- **Cinematic lighting (M25).** The whole arena sits under a per-map dark grade that dynamic light fights back against: every muzzle flash, projectile, beam, explosion, hit, death pillar, crate, and hazard lamp is a real light source that also lights the pilots. Fortress sweeps two searchlight cones that cast true occlusion shadows; the foundry furnace breathes from below; the canopy storms with strobe lightning and rolling thunder. An adaptive governor sheds shadow casting and decorative lights first when frames run long, and the full system is toggleable under `FX` ("Dynamic lighting / 动态光影").
...
- **Immersive rendering.** Generated bitmap environments with baked platform textures and material overlays, per-map atmosphere particles (canopy rain that splashes off platforms, fortress dust, factory embers and steam vents), and layered armored pilot sprites with per-archetype silhouettes. The arena renders at 1.3× resolution and zooms to match — everything is drawn larger and sharper with zero change to physics or balance. The procedural renderer remains as a guaranteed fallback when assets are missing.
...
`test:logic` checks deterministic limb penalties, hit regions, the swept-capsule hit geometry (tunneling regressions, capsule coverage, segment distance math), the movement budget (jump apex above the navigation rise limit), lift and mover travel, hazard phases, navigation-graph connectivity, map authoring rules, and the bot cliff-guard veto table. `test:smoke` verifies room creation, joining, settings, authoritative match start, weapon-slot switching (including slot 7 and over-range clamping), ranged hits, ricochet projectiles, solid-cover blocking, disconnect slot retention, reconnect, the solo sandbox lifecycle, bot roster materialization, bot activity, host migration away from bots, sandbox-with-bots, elimination freeze (no post-death simulation, no repeated death events, prompt match resolution), and immediate `leave_room` slot release. `test:ai` is the AI stability suite: bot free-for-alls must resolve on every map (backed by the 4-minute time limit), bots must engage an idle human, survive the layouts alone, never lock up mid-charge, and stay off fall gaps under sustained fire (a cliff-guard pressure test that plasters a ledge-born bot with ricochet fire for 30 seconds). `test:browser` covers multiplayer and solo UI flows, three maps, J/K, the Tab weapon panel with all seven weapons, the kill feed container, visual preferences, combat-feedback events (beam, pellet impacts, rocket explosions) with screenshots, refresh recovery, and the default-Chinese locale guarantee (suites pin English via a test helper). `test:visual` checks the menu, lobby, canvas, HUD, overflow, and nonblank rendering at four desktop viewports. `test:performance` drives four isolated clients through simultaneous J/K attacks and records frame timing plus a load screenshot.
